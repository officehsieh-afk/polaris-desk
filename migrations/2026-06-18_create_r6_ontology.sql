...
CLUSTER BY event_id
OPTIONS(description='R6 Ontology_V1 04_disclosure_events 揭露事件類型。');
...
CREATE OR REPLACE TABLE `polaris-desk-team.polaris_core.r6_risk_signal` (
  ...
  related_event_id  STRING    -- 對映 r6_disclosure_event.event_id
)
CLUSTER BY risk_id
...
-- 預期：industry=18, financial_metric=22, disclosure_event=13, compliance_term=38,
--       theme=17, risk_signal=22, company_theme_map=55, company_industry_map=27,
--       news_source_whitelist=13, source_taxonomy_map=14, revenue_field=6, quarter=12。
--
