...
**由 R3 加 `mode` 參數**，前端直接渲染回傳結果，不做前端字串轉換。

理由：措辭邏輯屬於後端 prompt 管理範疇；前端加前綴的方式會造成前後端耦合，未來 R3 調整建議措辭時前端也需同步修改。
...
> **R3 負責計算衍生財務指標，R4 只儲存原始數據。**
>
> `financial_metrics` 表的 `metric_id` 有 `revenue`、`gross_profit`、`operating_income` 等原始值，但沒有毛利率、營業利益率等比率。這些衍生指標由 R3 在 workflow 內計算：
>
...
建議回傳近 4–6 季，由 `financial_metrics` 查詢 `metric_id = "gross_margin"`，依 `fiscal_period` 排序。
