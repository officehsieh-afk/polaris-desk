...
- [x] D24 G4：確保 4 場景**在雲端**可重現 — **PASS（2026-06-16）**：對 live service 跑 S1 `/ask` factual 8/8、S2 `/ask` compare 8/8、S3 `/research` 6/6（answered/≤6 迴圈/≥3 evidence）、S4 `/alerts` 3/3；每場景 distinct signature=1（temp=0 + 結構保證，Vertex LLM 生成下仍 byte-stable）、`answer_is_real_llm=True`、0 買賣建議、compliance passed
